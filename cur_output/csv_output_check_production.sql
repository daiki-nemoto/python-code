CREATE EXTERNAL TABLE IF NOT EXISTS csvtest(
...
`cp_cost` string,
`cp_discount_rate` string,
`cp_discounted_cost` string,
...
`product_api_calls` string,
`product_api_category` string,
`product_api_type` string,
`product_apirequestsfree` string,
`product_apirequestspaid` string,
`product_architectural_review` string,
`product_architecture_support` string,
`product_attachment_type` string,
`product_automatic_label` string,
`product_availability` string,
`product_availability_zone` string,
`product_awsiotevents` string,
...
`product_cachememorysize` string,
...
`product_changerequestsfree` string,
`product_changerequestspaid` string,
...
`product_cloudformationresource_provider` string,
...
`product_destination_country_iso_code` string,
`product_detail` string,
`product_device` string,
`product_device_os` string,
...
`product_io` string,
`product_is_spot_instance` string,
`product_isfree` string,
`product_isshadow` string,
`product_iswebsocket` string,
`product_labeling_task_type` string,
...
`product_lockeprofiles` string,
`product_logs_destination` string,
`product_lookoutvisionimage` string,
...
`product_message_countfee` string,
`product_message_delivery_frequency` string,
`product_message_delivery_order` string,
`product_message_type` string,
`product_meter_mode` string,
`product_metering_type` string,
`product_meterunit` string,
...
`product_platoresourceactionmetrics` string,
`product_platostoragename` string,
`product_platostoragetype` string,
`product_platotasktype` string,
...
`product_request_count` string,
`product_request_description` string,
`product_request_duration` string,
`product_request_type` string,
`product_reserve_type` string,
...
`product_storage_capacity_quota` string,
...
`resource_tags_user_aws_migration_project_id` string,
`resource_tags_user_chorus_cost_tag1` string,
`resource_tags_user_chorus_cost_tag2` string,
`resource_tags_user_chorus_cost_tag3` string,
`resource_tags_user_chorus_cost_tag4` string,
`resource_tags_user_chorus_cost_tag5` string,
`resource_tags_user_map_dba` string,
`resource_tags_user_map_migrated` string,
`resource_tags_user_map_migrated_app` string,
`resource_tags_user_map_sap` string,
...
`year` string
 ...
 LOCATION 's3://chorus-portal-shared-bucket-for-751/chorus_portal/2024/03/20240321T132131Z/'
